...
int main(int argc, char** argv)
{
	...
	if (RECORD_VIDEO)
		outputVideo.open("img/test.avi", CV_FOURCC('D', 'I', 'V', 'X'), 30.0, cv::Size(IMG_WIDTH * 2, IMG_HEIGHT * 3), false);
	...
	do {
		...
		if (RECORD_IMAGES){
			ss.str("");
			ss << "img/Left_" << imageBuffer.getFrameCount() << ".png";
			cv::imwrite(ss.str(), imageBuffer.getImage(0, LEFT_IMG));

			ss.str("");
			ss << "img/Diff_" << imageBuffer.getFrameCount() << ".png";
			cv::imwrite(ss.str(), imageBuffer.getImage(0, LEFT_DIFF));
		}
		...
		/* previous images */
		imageBuffer.getImage(0, LEFT_IMG).copyTo(windowImage(cv::Rect(0, IMG_HEIGHT, IMG_WIDTH, IMG_HEIGHT)));
		imageBuffer.getImage(0, RIGHT_IMG).copyTo(windowImage(cv::Rect(IMG_WIDTH, IMG_HEIGHT, IMG_WIDTH, IMG_HEIGHT)));
